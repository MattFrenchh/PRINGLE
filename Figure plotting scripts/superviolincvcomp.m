...
%     hp(k)=patch('XData',hvalh(:)+p.Xposition, 'YData', edgeh(:),'FaceColor',colors(k,:));
    hp(k)=patch('XData',hvalh(:)+p.Xposition, 'YData', edgeh(:),'FaceColor',p.colour,...
                      'EdgeColor', 'w');
...
hold(ax,'on')
for k=1:numhist
%     plot(ax,xp(k)+p.Xposition,yp(k),'ko','MarkerFaceColor',p.colour,...
    plot(ax,xp(k)+p.Xposition,yp(k),'ko','MarkerFaceColor','black',...
        'LineWidth',2*p.LineWidth,'MarkerSize',2, 'Color', 'black');
%         'LineWidth',2*p.LineWidth,'MarkerSize',3, 'Color', 'none');
...
errorbar(p.Xposition,mhistall,sall(1),sall(2),'k','LineWidth',2,'CapSize',8, 'Color', 'black');
errorbar(p.Xposition,mhistall,0.00,'k','LineWidth',2,'CapSize',16, 'Color', 'black');
%
end
