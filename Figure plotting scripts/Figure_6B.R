...
hNMP_data <- as.data.frame(read_excel("S2_Data.xlsx", sheet = "hNMP_D3_Data", col_names = TRUE))
...
#Ensure that the columns are numeric if they are not already
Wt_E8_data$log_SOX2 <- as.numeric(Wt_E8_data$log_SOX2)
Wt_E8_data$log_TBX6 <- as.numeric(Wt_E8_data$log_TBX6)

# Get the local density of points in PCA space for FACS style plot
...
#Include only CHIR 2 or 3 micro-molar for hNMP data
