...
for k=1:numhist
    hvalh=[histp(:,k) histp(end:-1:1,k+1)];
    edgeh=[sampledpoints sampledpoints(end:-1:1)];
    hp(k)=patch('XData',hvalh(:)+p.Xposition, 'YData', edgeh(:),'FaceColor','black');
    hp(k).FaceAlpha=p.FaceAlpha;
    hp(k).LineWidth=p.LineWidth;
end
...
%         'LineWidth',2*p.LineWidth,'MarkerSize',12);
% end

for k=1:numhist
    plot(ax,xp(k)+p.Xposition,yp(k),'ko','MarkerFaceColor','white',...
        'LineWidth',3*p.LineWidth,'MarkerSize',4);
end
%
...
errorbar(p.Xposition,mhistall,sall(1),sall(2),'k','LineWidth',3,'CapSize',20);
errorbar(p.Xposition,mhistall,0.00,'k','LineWidth',3,'CapSize',30);
errorbar(p.Xposition,mhistall,0.00,'o',"MarkerSize",10,"MarkerFaceColor",'white', 'Color', 'black');
